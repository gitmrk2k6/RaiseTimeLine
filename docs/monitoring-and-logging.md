...
- AWS 本番環境（EC2 + RDS + ALB）での **安定運用** を支援する
...
| AWS CloudWatch Logs | ログ収集・検索・アラーム |
| Actuator | ヘルスチェック（ALB 用） |
...
JSON 形式でコンソールと `/var/log/raisetimeline/app.log` に出力。CloudWatch Logs Agent が収集する。
...
| EC2 CPU 使用率（15分平均） | > 70% | > 90% | CloudWatch EC2 メトリクス |
| ディスク使用率（EC2） | > 70% | > 85% | CloudWatch Agent カスタムメトリクス |
...
| D | レスポンスタイム悪化のみ | EC2 / RDS の CPU・メモリ使用率を確認 |
...
```bash
sudo systemctl restart raisetimeline
```
...
EC2 に CloudWatch Agent を設定し `/var/log/raisetimeline/app.log` を収集する。

設定ファイル (`/etc/amazon/amazon-cloudwatch-agent/amazon-cloudwatch-agent.json`):
...
```json
{
  "logs": {
    "logs_collected": {
      "files": {
        "collect_list": [
          {
            "file_path": "/var/log/raisetimeline/app.log",
            "log_group_name": "/raisetimeline/prod/backend",
  ...
  "metrics": {
    "namespace": "RaiseTimeLine/EC2",
    "metrics_collected": {
      "cpu": {
        "measurement": ["cpu_usage_user", "cpu_usage_idle"],
...
| filter message like /HTTP / and durationMs > 3000
